...
_ids=json.load(open('data/ids.json')); IDS=_ids['curated']+_ids['auto']
os.makedirs('data/bd', exist_ok=True)
def work(mid):
    d=get(f'/movie/{mid}/images')
    if not d: return mid, []
    bds=sorted(d['backdrops'], key=lambda b:(b.get('iso_639_1') is not None, -b.get('vote_count',0), -b.get('vote_average',0)))[:10]
    out=[]
    for b in bds:
    ...
    return mid, out
res=json.load(open('data/backdrops.json')) if os.path.exists('data/backdrops.json') else {}
IDS=[i for i in IDS if str(i) not in res]
print('to fetch',len(IDS))
with cf.ThreadPoolExecutor(12) as ex:
    for mid,out in ex.map(work, IDS): res[str(mid)]=out
